...
<ul>
	<c:choose>
		...
		<c:when test="${step eq '8'}">
			<li>
				...
				URL 2 : <c:url var="link2" value="/temp/jstlImport.do">
				            <c:param name="test" value="텍스트 호출"/>
				        </c:url>
				        <br/>
		...
		<c:when test="${step eq '9'}">
			<li>
				<h3>c:fn:contains() : 주어진 문자열이 특정 문자열을 포함하고 있는지 확인하는데 사용</h3>
		...
		<c:when test="${step eq '10'}">
			<li>
				<h3>c:fn:indexOf() : 주어진 문자열이 특정 문자열을 포함하면 주어진 문자열 내에서 인덱스를 변환</h3>
		...
		<c:when test="${step eq '11'}">
			<li>
				<h3>c:fn:startsWith() : 주어진 문자열이 특정 문자열 값으로 시작되는지 확인하는데 사용</h3>
		...
		<c:when test="${step eq '12'}">
			<li>
				<h3>c:fn:endsWith() : 주어진 문자열이 특정 문자열 값으로 끝나는지 확인하는데 사용</h3>
		...
		<c:when test="${step eq '14'}">
			<li>
				...
				<c:set var="strl" value="This is first string"/>
				<c:set var="str" value="1,2,3,4,5,6"/>
				<c:set var="strSplit" value="${fn:split(str,',')}"/>
				Length of the String-1 is: <c:out value="${fn:length(str)}"/><br>
		...
		<c:when test="${step eq '19'}">
			<li>
				<h3>fn:trim() : 문자열 양쪽 끝의 공백을 제거(문자열 사이의 공백은 사라지지 않음)</h3>
				
				<c:set var="strl" value="      Welcome to MSJ class       "/>
				<p>String-1 Length is : ${fn:length(strl)}</p>
				
				<c:set var="strl2" value="${fn:trim(strl)}"/>
