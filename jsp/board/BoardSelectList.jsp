...
<div class="container">
	<div id="contents">
			...
			<form name="frm" method="post" action="/board/selectList.do">
				<fieldset>
					...
					<select name="searchCondition" id="ftext">
						<option value="0" <c:if test="${searchVO.searchCondition eq '0'}">selected="selected"</c:if>>목록</option>
		...
		<div id="bbs_wrap">
						...
						<ul>
							<c:forEach var="result" items="${resultList}" varStatus="status">
								<c:url var="viewUrl" value="/board/select.do${_BASE_PARAM}">
									<c:param name="baordId" value="${result.boardId}"/>
										...
										<a href=${viewUrl}">
											<c:choose>
												<c:when test="${empty result.atchFileNm}">
													<img src="/asset/BBSTMP_0000000000001/images/noimg.gif" width="200" height="140"
														alt="<c:out value="${result.boardSj}"/>"/>
												</c:when>
												<c:otherwise>
										...
										<span>
											<a href="${viewUrl}">
												<c:if test="${result.othbcAt eq 'Y'}">
													<img src="/asset/BBSTMP_0000000000001/image/ico_board_lock.gif" alt="비밀 글 아이콘" />
												</c:if>
												<c:choose>
						...
						<table class="list_table">
							...
							<tbody>
								...
								<c:forEach var="result" items="${resultList}" varStatus="status">
									<tr>
										<td>
											<c:out value="${paginationInfo.totalPageCount - ((searchVO.pageIndex-1) * searchVO.pageUnit) - (status.count - 1)}"/>
										</td>
										<td class="tit">
											<c:url var="viewUrl" value="/board/select.do${_BASE_PARAM}">
												<c:param name="boardId" value="${result.boardId}"/>
			...
			<div id="paging">
				<c:url var="pageUrl" value="/board/select.do${_BASE_PARAM}"/>
